<template>
  <a-modal
    ...
    :visible="visible"
  >
    <a-form-model class="my-form" ref="ruleForm" :model="ruleForm" :rules="rules" v-bind="layout">
      <a-form-model-item label="账号" prop="userName">
        <a-input v-model="userName" type="text" autocomplete="off" disabled />
      </a-form-model-item>
      ...
      <a-form-model-item label="新密码" prop="newPassword">
        ...
        <div class="psw-block" v-if="ruleForm.newPassword">
          <div class="ps-child">
            <span class="p-colors" :style="{'background':userStyleInput>=1?'red':'#ccc'}"></span>
            <span class="p-text">弱</span>
          </div>
          <div class="ps-child">
            <span class="p-colors" :style="{'background':userStyleInput>=3?'blue':'#ccc'}"></span>
            <span class="p-text">中</span>
          </div>
          <div class="ps-child">
            <span class="p-colors" :style="{'background':userStyleInput>=4?'green':'#ccc'}"></span>
            <span class="p-text">强</span>
          </div>
        </div>
        <span class="text" v-if="ruleForm.newPassword">提示：密码由数字、小写字母、大写字母、特殊字符4种类型组成</span>
      </a-form-model-item>
      <a-form-model-item has-feedback label="密码确认" prop="confirmPassword">
      ...
      >
        ...
        <a-button style="margin-left: 10px" @click="hide">取消</a-button>
      </a-form-model-item>
    </a-form-model>
  </a-modal>
</template>
...
export default class App extends Vue {
  ...
  private get userStyleInput() {
    let value = this.ruleForm.newPassword;
  ...
  private passwordFun(rule: unknown, value: string, callback: any) {
    let datas = this.ruleForm.newPassword;
    let modes = checkPassModes(datas);
